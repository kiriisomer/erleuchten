...
# Try to find the order of VMs to perform interface-off
num_of_fields=`awk '$1 == "INTERFACE-ORDER" {print NF - 1;}' ../shells.conf`
check_number_of_fields 3 $num_of_fields

VMs[0]=`awk '$1 == "INTERFACE-ORDER" {print int($2);}' ../shells.conf`
VMs[1]=`awk '$1 == "INTERFACE-ORDER" {print int($3);}' ../shells.conf`
VMs[2]=`awk '$1 == "INTERFACE-ORDER" {print int($4);}' ../shells.conf`
...
# Find the time to stay interface-off
num_of_fields=`awk '$1 == "INTERFACE-OFF" {print NF - 1;}' ../shells.conf`
check_number_of_fields 1 $num_of_fields

off_time=`awk '$1 == "INTERFACE-OFF" {print int($2);}' ../shells.conf`

# Find the interval during each interface-off-then-on
num_of_fields=`awk '$1 == "INTERFACE-OFF" {print NF - 1;}' ../shells.conf`
check_number_of_fields 1 $num_of_fields

wait_time=`awk '$1 == "INTERFACE-INTERVAL" {print int($2);}' ../shells.conf`
...
erleuchten-env sshcmd --name $ENV --order 1 \
	--cmd "echo 'This is the file of interface-test' > /mnt/test/1001/test.txt"
erleuchten-env sshcmd --name $ENV --order 1 \
...
for (( tmp=0; tmp<=2;tmp++ )); do
	erleuchten-env disconnect-if  --name $ENV --order ${VMs[$tmp]} \
		--interface storageif${VMs[$tmp]}
	for((i=1; i<=$off_time; i++)); do
		echo "-->>$used_time/$total_time-Interface-Test-Order:${VMs[$tmp]}<<--"
		used_time=$[used_time + 1]
		sleep 1
	done

	erleuchten-env connect-if  --name $ENV --order ${VMs[$tmp]} \
		--interface storageif${VMs[$tmp]}
